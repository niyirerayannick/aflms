...
--------------------------------------------
1 or YES or ACCEPT   → Accept the current assigned trip
2 or NO or DECLINE   → Decline the current assigned trip
3 or START            → Start the trip (then send KM reading)
<number>              → KM reading (when awaiting)
4 or DONE/DELIVERED   → Mark trip delivered
FUEL <liters>         → Request fuel for current trip
APPROVE <id>          → Approve fuel request (managers)
REJECT <id>           → Reject fuel request (managers)
STATUS                → Check current trip status
HELP or HI or MENU   → Show available commands
"""
...
# ---------------------------------------------------------------------------
# Session store – maps phone → context (current trip, pending action, etc.)
...
def _handle_accept(phone: str, order_number: str = None) -> str:
    ...
    if not driver:
        return "❌ Your phone number is not linked to a driver account. Contact admin."
    ...
    _notify_managers(
        f"✅ Driver {driver.name} ACCEPTED trip {trip.order_number}."
    )
    ...
    return (
        f"✅ You accepted trip *{trip.order_number}*\n\n"
        f"📍 {trip.route.origin} → {trip.route.destination}\n"
        f"🚛 {trip.vehicle.plate_number}\n"
        f"📦 {trip.customer.company_name}\n\n"
...
def _handle_decline(phone: str, order_number: str = None) -> str:
    ...
    _notify_managers(
        f"⚠️ Driver {driver.name} DECLINED trip {trip.order_number}. "
        "Please reassign."
    )
    _sessions.pop(phone, None)
    return f"❌ You declined trip *{trip.order_number}*. A manager has been notified."
...
def _handle_start(phone: str, order_number: str = None) -> str:
    ...
    # Store session – waiting for KM reading
    _sessions[phone] = {"action": "awaiting_km", "trip_id": trip.pk}
    return (
        f"🚀 Starting trip *{trip.order_number}*\n\n"
...
def _handle_km(phone: str, reading_str: str) -> str:
    ...
    try:
        ...
        return "❌ Invalid KM reading. Please send a number, e.g. *125430*"
    ...
    return (
        f"🚚 Trip *{trip.order_number}* is now *IN TRANSIT*\n"
...
def _handle_delivered(phone: str, order_number: str = None) -> str:
    ...
    return (
        f"📍 Confirming delivery for *{trip.order_number}*\n\n"
...
def _handle_km_end(phone: str, reading_str: str) -> str:
    ...
    try:
        ...
        return "❌ Invalid KM reading. Please send a number, e.g. *125890*"
    ...
    if km_reading < trip.km_start:
        return (
            f"❌ Odometer end (*{km_reading}*) cannot be less than "
    ...
    _notify_managers(
        f"📦 Trip {trip.order_number} marked DELIVERED by {driver.name}.\n"
        f"Distance: {trip.distance} km (KM {trip.km_start} → {trip.km_end})"
    )
    _sessions.pop(phone, None)
    return (
        f"✅ Trip *{trip.order_number}* marked as *DELIVERED*\n\n"
        f"🛣️ KM: {trip.km_start} → {trip.km_end}\n"
        f"📏 Distance: *{trip.distance} km*\n\n"
        f"Thank you, {driver.name}! 🎉"
    )
...
def _handle_fuel_request(phone: str, order_number: str = None, liters_str: str = "") -> str:
    ...
    try:
        ...
        return "❌ Invalid liter amount. Example: *FUEL 100*"
    ...
    return (
        f"⛽ Fuel request submitted: *{liters}L*\n"
        f"Trip: {trip.order_number}\n"
...
def _handle_approve(phone: str, request_id_str: str) -> str:
    ...
    if not user:
        return "❌ You are not authorized to approve fuel requests."
...
def _handle_reject(phone: str, request_id_str: str) -> str:
    ...
    if not user:
        return "❌ You are not authorized to reject fuel requests."
    ...
    send_whatsapp_message(
        fuel_req.driver.phone,
        f"❌ Your fuel request #{fuel_req.pk} has been *REJECTED*.\n"
        f"Trip: {fuel_req.trip.order_number}\n"
        f"Contact your manager for details.",
    )
    return f"❌ Fuel request #{fuel_req.pk} rejected."
...
def _handle_status(phone: str, order_number: str = None) -> str:
    ...
    if order_number:
        ...
        return "❌ Your phone number is not linked to a driver account."
    ...
    return (
        ...
        f"KM: {trip.km_start} → {trip.km_end or '—'}\n"
...
def _handle_help(phone: str) -> str:
    ...
    menu += (
        "\n*All Commands:*\n"
        "*1* or *YES* — Accept trip\n"
        "*2* or *NO* — Decline trip\n"
        "*3* or *START* — Start trip\n"
        "*4* or *DONE* — Mark delivered\n"
        "*FUEL <liters>* — Request fuel\n"
...
# Regex patterns for command matching — numbered shortcuts come first
_PATTERNS = [
    ...
    # Fuel request — "FUEL 100" or "FUEL REQUEST <order> <liters>"
